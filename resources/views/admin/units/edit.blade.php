...
@section('title', 'Ubah Unit — SIMARS-DOC')
...
<div class="max-w-xl mx-auto">
    ...
    <div class="mb-6">
        ...
        <h1 class="text-2xl font-bold text-gray-900">Ubah Unit</h1>
        <p class="text-sm text-gray-500 mt-0.5">{{ $unit->code }} — {{ $unit->name }}</p>
    </div>
    ...
    <form method="POST" action="{{ route('admin.units.update', $unit) }}">
        ...
        <div class="bg-white border border-gray-200 rounded-xl shadow-sm p-6 mb-4 space-y-4">
                ...
                <div class="ina-text-field">
                    ...
                    <div class="ina-text-field__wrapper">
                        <input type="text" name="code" class="ina-text-field__input @error('code') border-red-500 @enderror"
                            value="{{ old('code', $unit->code) }}" placeholder="misal: IGD" style="text-transform: uppercase;">
                    </div>
                ...
                <div class="ina-text-field">
                    ...
                    <div class="ina-text-field__wrapper">
                        <input type="text" name="name" class="ina-text-field__input @error('name') border-red-500 @enderror"
                            value="{{ old('name', $unit->name) }}" placeholder="Nama lengkap unit">
                    </div>
            ...
            <div class="ina-text-field">
                    ...
                    <select name="parent_id" class="ina-text-field__input @error('parent_id') border-red-500 @enderror">
                        ...
                        @foreach ($parentUnits as $parent)
                            <option value="{{ $parent->id }}"
                                {{ old('parent_id', $unit->parent_id) === $parent->id ? 'selected' : '' }}>
                ...
                @error('parent_id')
                    <p class="ina-text-field__helper-text text-red-500">{{ $message }}</p>
                @else
                    <p class="ina-text-field__helper-text text-gray-500">
                        Unit ini beserta turunannya tidak ditampilkan agar hierarki tidak melingkar.
                    </p>
                @enderror
            </div>
